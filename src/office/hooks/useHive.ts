...
export function useHive(config: HarnessConfig | null): void {
  ...
  useEffect(() => {
    ...
    void window.cth.hiveRegistry().then((reg) => {
      ...
      const agentEntries = (reg?.agents ?? {}) as Record<string, any>;
  ...
  useEffect(() => {
    ...
    type SlackTaskCard = { id: string; title?: string; description?: string; status?: string; dependsOn?: any[]; priority?: number; [key: string]: any };
