...
import { useStore, type OfficeStore } from '../store/store';
        ...
        <div style={{ fontSize: 11, color: 'var(--cth-ink-500)', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          {tool.docsUrl && (
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                if (typeof window !== 'undefined' && window.cth?.openExternal) {
                  void window.cth.openExternal(tool.docsUrl!);
                } else if (tool.docsUrl) {
...
export function SetupPanel({ onDone }: { onDone?: () => void } = {}) {
  ...
  const requestDispatchSeed = useStore((s: OfficeStore) => s.requestDispatchSeed);
  const requestCommandCenterTab = useStore((s: OfficeStore) => s.requestCommandCenterTab);

  const refresh = useCallback(async () => {
    setBusy(true);
    try {
      const res = typeof window !== 'undefined' && window.cth?.toolsStatus ? await window.cth.toolsStatus() : [];
      setTools(res ?? []);
    }
    catch { setTools([]); }
