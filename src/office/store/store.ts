...
const DEFAULT_MAHR_AGENTS: Agent[] = [
  {
    id: 'agent-mahr',
    name: 'MAHR',
    character: 'michael',
    accent: 'lilac',
    status: 'working',
    action: 'orchestrating office floor',
    progress: 4,
    currentStation: 'desk',
    description: "The boss of MAHR Office. Directs subordinate agents, delegates tasks, and maintains office harmony.",
    project: 'MAHR Core',
    tmuxTarget: 'mahr:0',
    cwd: '.',
    isGod: true,
  ...
  },
  {
    ...
    accent: 'sky',
    status: 'working',
    action: 'polishing React UI components',
    progress: 3,
    currentStation: 'shelf',
    carrying: 'Edit',
    description: "Prankster, problem-solver, and pixel-art specialist.",
    project: 'MAHR Frontend',
    tmuxTarget: 'mahr:1',
    cwd: 'src/office',
    provider: 'claude'
  },
  {
    ...
    accent: 'lemon',
    status: 'working',
    action: 'running security & type audits',
    progress: 5,
    currentStation: 'terminal',
    carrying: 'Bash',
    description: "Assistant to the Regional Manager. Strict compliance, zero hallucinations.",
    project: 'MAHR Security',
    tmuxTarget: 'mahr:2',
    cwd: 'src',
    provider: 'claude'
  },
  {
    ...
    accent: 'coral',
    status: 'thinking',
    action: 'routing memos & design specs',
    progress: 2,
    currentStation: 'board',
    carrying: 'TodoWrite',
    description: "Connects students and developers to the office workflow.",
    project: 'MAHR Design',
    tmuxTarget: 'mahr:3',
    cwd: 'src/components',
    provider: 'claude'
  },
  {
    ...
    accent: 'mint',
    status: 'idle',
    action: 'brainstorming tech stack',
    progress: 1,
    currentStation: 'web',
    carrying: 'WebFetch',
    description: "Temp turned startup enthusiast.",
    project: 'MAHR APIs',
    tmuxTarget: 'mahr:4',
    cwd: '.',
    provider: 'cursor'
  },
  {
    ...
    accent: 'peach',
    status: 'idle',
    action: 'monitoring system latency',
    progress: 2,
    currentStation: 'desk',
    description: "Crossword champion, ensuring sub-millisecond query responses.",
    project: 'MAHR Database',
    tmuxTarget: 'mahr:5',
    cwd: '.',
    provider: 'codex'
  }
...
function loadPersistedAgents(): Agent[] {
  try {
    // Purge any stale 'reconnecting…' strings from localStorage cache
    try {
      const raw = window.localStorage.getItem(LS_AGENTS);
      if (raw && raw.includes('reconnecting')) {
        const cached = JSON.parse(raw);
        if (Array.isArray(cached)) {
          const defMap = new Map(DEFAULT_MAHR_AGENTS.map((d) => [d.id, d]));
          const sanitized = cached.map((a: any) => {
            const def = defMap.get(a.id) || DEFAULT_MAHR_AGENTS.find((d) => d.character === a.character);
            const rawAct = (a.action || '').trim();
            const isReconn = !rawAct || /^reconnecting/i.test(rawAct);
            return {
              ...a,
              action: isReconn ? (def?.action || 'ready at station') : rawAct,
              status: (a.status && a.status !== 'reconnecting') ? a.status : (def?.status || 'idle')
            };
          });
          window.localStorage.setItem(LS_AGENTS, JSON.stringify(sanitized));
        }
      }
    } catch { /* noop */ }

    const parsed = persistedSlice(LS_AGENTS, fileRoster?.agents);
    if (!parsed.length) return DEFAULT_MAHR_AGENTS;
    ...
    return parsed.map((a) => {
      ...
      const rawAct = (a.action || '').trim();
      const isReconn = !rawAct || /^reconnecting/i.test(rawAct);
      const naturalAction = isReconn ? (def?.action || 'ready at station') : rawAct;
      const naturalStatus = (!a.status || a.status === ('reconnecting' as any)) ? (def?.status || 'idle') : a.status;

      return {
        ...a,
        progress: a.progress ?? (def?.progress || 0),
        status: naturalStatus,
        action: naturalAction,
        currentStation: a.currentStation || def?.currentStation || 'desk',
        carrying: a.carrying ?? def?.carrying,
        recentTextTs: Date.now(),
      };
    });
  } catch {
    return DEFAULT_MAHR_AGENTS;
  }
}
